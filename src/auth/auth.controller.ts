...
export class AuthController {
    ...
    async login(@Body('email') email: string,
                @Body('password') password: string): Promise<{accessToken: string, refreshToken: string}> {
    ...
    async refreshToken(@Body('refresh_token') refresh: string,
                       @Headers('authorization') authorization): Promise<{accessToken: string, refreshToken: string}> {
        const accessToken: any = this._service.verifyToken(authorization, true);
        const refreshToken: RefreshTokensEntity = await this._refreshTokenService.get(refresh);
